#!/usr/bin/env python3
"""Build a bounded ZipCPU query from the *entire* original Yosys SMT model.
Do not remove zipcpu_h, transition, initialization, or any assertions except the
single explicitly identified historical address assertion in parent_relaxed.
"""
...
A=P.parse_args()
assert A.depth>=A.last+1>=A.first+1
original=A.model.read_text()
s=original
assert '; yosys-smt2-topmod zipcpu' in s
assert 'dblfetch/ADDRESS_WIDTH=30' in s and 'mpyop' in s
# The real conflict is dblfetch.v:350, NOT the initial-reset assumption on line 355.
...
# Exactly the historical native testbench's Wishbone response: prior request
# acknowledged one edge later; data is the same fixed ROM, held without request.
...
for t in range(A.first,A.last+1):
    ...
    if A.case=='parent_relaxed':xs.append('(not ('+address+' (|zipcpu_h domem| s'+str(t+1)+')))')
    target.append('(and '+' '.join(xs)+')')
lines.append(put('(or '+' '.join(target)+')'))
lines.append('(check-sat)\n')
# The solver's SAT witness includes the words actually fetched and the failing
# assertion value; do not trust a synthetic source/trace marker instead.
for t in range(A.depth+1):
...
print('CASE',A.case,'DEPTH',A.depth,'TARGET',A.first,A.last)
