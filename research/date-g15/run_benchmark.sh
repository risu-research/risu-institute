...
# Genuine pinned benchmark sources, never hand-written substitutes.
...
# Fixed, untuned strong compiler baselines. Complete population is required.
for variant in Os O2 O3 OsLTO; do
  ...
  flags="--specs=$PICO_SPECS -march=rv32imac -mabi=ilp32 $opt -ffunction-sections -fdata-sections"
  ...
  # Upstream build_all.py can return zero when every source failed. Reject it.
  if grep -E 'Warning: (Compilation|Linking).*failed' "$OUT/build-logs/${variant}.stdout.log" >/dev/null; then
    ...
    grep -E -m 5 -A 6 'Warning: (Compilation|Linking).*failed' "$OUT/build-logs/$variant"/build-*.log >&2 || true
    exit 22
  fi
