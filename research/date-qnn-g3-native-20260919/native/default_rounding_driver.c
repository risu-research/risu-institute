...
static void num(int v){char b[16];int j=0;unsigned x;if(v<0){b[j++]='-';x=(unsigned)(-v);}else x=(unsigned)v;char tmp[11];int n=0;do{tmp[n++]=(char)('0'+x%10);x/=10;}while(x)b[j++]=tmp[--n];b[j]=0;puts0(b);}
