...
static int run(const char*label,int input,int offset,int shift,int n,int expected_scalar,int expected_mve){
 int8_t a[8],b[8],s[8],v[8];int fail=0;
 for(int i=0;i<8;i++){a[i]=(int8_t)input;b[i]=(int8_t)input;s[i]=v[i]=(int8_t)77;}
 int rc_s=scalar_kernel(a,b,offset,offset,s,0,1073741824,shift,-100,100,n);
 int rc_v=mve_kernel(a,b,offset,offset,v,0,1073741824,shift,-100,100,n);
...
int main(void){int errors=0;
 errors+=run("CONTROL_SHIFT14",100,100,14,4,100,100);
 errors+=run("WITNESS_SHIFT15",100,100,15,4,100,-100);
 errors+=run("TAIL_SHIFT15",100,100,15,5,100,-100);
 errors+=run("BOUNDARY_SHIFT15",127,128,15,4,100,-100);
